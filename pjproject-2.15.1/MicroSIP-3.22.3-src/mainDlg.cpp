...
BEGIN_MESSAGE_MAP(CmainDlg, CBaseDialog)
	...
	ON_COMMAND(ID_CLIENT_CARD, OnMenuClientCard)
...
BOOL CmainDlg::OnInitDialog()
{
	...
	AutoMove(IDC_MAIN_LOGO, 0, 0, 100, 0);
...
void CmainDlg::OnMenuClientCard()
{
	OpenClientCardForCall(CurrentCallId());
}
...
void CmainDlg::MainPopupMenu(bool isMenuButton)
{
	...
	CMenu* tracker = &menu;
	pjsua_call_id activeCallId = CurrentCallId();
	if (activeCallId != PJSUA_INVALID_ID && pjsua_call_is_active(activeCallId)) {
		tracker->AppendMenu(MF_STRING, ID_CLIENT_CARD, Translate(_T("Client Card")));

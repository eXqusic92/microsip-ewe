...
void RinginDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ANSWER, m_ButtonAnswer);
	DDX_Control(pDX, IDC_DECLINE, m_ButtonDecline);
}
...
BOOL RinginDlg::OnInitDialog() {
	...
	AutoMove(IDC_CLIENT_CARD, 0, 100, 0, 0);
	...
	m_ButtonAnswer.m_FaceColor = _GLOBAL_DIALER_CALL_COLOR;
	m_ButtonAnswer.m_TextColor = RGB(255, 255, 255);
	m_ButtonAnswer.m_nAlignStyle = CMFCButton::ALIGN_CENTER;
	m_ButtonAnswer.m_bRightImage = FALSE;
	m_ButtonAnswer.SetImage(LoadImageIcon(IDI_CALL_ACCEPT, 16, 16), FALSE, NULL, NULL, TRUE);
	m_ButtonAnswer.EnableWindow(m_ButtonAnswer.IsWindowEnabled());
	m_ButtonDecline.m_FaceColor = _GLOBAL_DIALER_END_COLOR;
	m_ButtonDecline.m_TextColor = RGB(255, 255, 255);
	m_ButtonDecline.m_nAlignStyle = CMFCButton::ALIGN_CENTER;
	m_ButtonDecline.m_bRightImage = FALSE;
	m_ButtonDecline.SetImage(LoadImageIcon(IDI_CALL_DECLINE, 16, 16), FALSE, NULL, NULL, TRUE);
	m_ButtonDecline.EnableWindow(m_ButtonDecline.IsWindowEnabled());
...
BEGIN_MESSAGE_MAP(RinginDlg, CBaseDialog)
	...
	ON_BN_CLICKED(IDC_CLIENT_CARD, &RinginDlg::OnBnClickedClientCard)
...
void RinginDlg::OnBnClickedClientCard()
{
	OpenClientCardForCall(call_id);
}

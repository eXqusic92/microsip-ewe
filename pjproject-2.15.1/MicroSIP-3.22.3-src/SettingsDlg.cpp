...
BOOL SettingsDlg::OnInitDialog()
{
	...
	GetDlgItem(IDC_SETTINGS_CLIENT_API_HOST)->SetWindowText(accountSettings.clientApiHost);
...
LRESULT SettingsDlg::OnUpdateSettings(WPARAM wParam, LPARAM lParam)
{
	...
	GetDlgItem(IDC_SETTINGS_CLIENT_API_HOST)->GetWindowText(accountSettings.clientApiHost);
	accountSettings.clientApiHost.Trim();
	combobox= (CComboBox*)GetDlgItem(IDC_SETTINGS_DEFAULT_ACTION);
...
void SettingsDlg::OnNMClickSyslinkForwarding(NMHDR *pNMHDR, LRESULT *pResult)
{
	OpenHelp(_T("forwarding"));
	*pResult = 0;
}

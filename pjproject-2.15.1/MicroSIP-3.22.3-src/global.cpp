...
{
    ParseCallSIPURI(MSIP::PjToStr(&call_info->remote_info, TRUE), user_data, out);
}
...
struct call_tonegen_data* call_init_tonegen(pjsua_call_id call_id)
{
	...
	pool = pjsua_pool_create("msip_tonegen", 512, 512);
...
void msip_set_sound_device(int outDev, bool forse) {
	...
	int in, out;
	if (forse || msip_audio_input == -1 || pjsua_get_snd_dev(&in, &out) != PJ_SUCCESS || msip_audio_input != in || outDev != out) {
...
void msip_call_dial_dtmf(pjsua_call_id call_id, CString digits)
{
	...
	if (call_id != PJSUA_INVALID_ID) {
		...
		if (call_info.media_status == PJSUA_CALL_MEDIA_ACTIVE) {
			...
			if (accountSettings.DTMFMethod == 1) {
				// in-band
				simulate = !call_play_digit(call_id, CStringA(digits));
			}
	...
	if (simulate && accountSettings.localDTMF) {
		...
		call_play_digit(-1, CStringA(digits));
	}
}
...
CString ClientCardUrl(CString number)
{
	...
	for (int i = 0; i < number.GetLength(); i++) {
		TCHAR character = number.GetAt(i);
	...
	CString host = accountSettings.clientApiHost;
	host.Trim();
	if (host.IsEmpty() || number.IsEmpty()) {
		return _T("");
	}
	if (host.Find(_T("://")) == -1) {
		host = _T("http://") + host;
	}
	while (host.Right(1) == _T("/")) {
		host = host.Left(host.GetLength() - 1);
	}

	CString url;
	url.Format(_T("%s/client-card?phone=%s"), host, CString(urlencode(MSIP::Utf8EncodeUni(number))));
	return url;
}

bool OpenClientCard(CString number)
{
	CString url = ClientCardUrl(number);
	if (url.IsEmpty()) {
		return false;
	}
	MSIP::OpenURL(url);
...
{
	pjsua_call_info call_info;
	if (!is_pjsua_running() || call_id == PJSUA_INVALID_ID ||
		pjsua_call_get_info(call_id, &call_info) != PJ_SUCCESS) {
		return false;
	}
...
CString get_public_addr(Account* account)
{
	...
	if (!res.IsEmpty()) {
		ULONG min = -1;
		ULONG ipn = inet_addr(CStringA(res).GetBuffer());
...
{
	//or Reason: Q.850 ;cause=17 ; text="User busy..."
	if (!reason.IsEmpty()) {

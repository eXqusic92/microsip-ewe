...
BOOL MessagesDlg::OnInitDialog()
{
	...
	AutoMove(IDC_CLIENT_CARD, 0, 0, 0, 0);
	...
	m_hIconCall = LoadImageIcon(IDI_CALL_ACCEPT, 16, 16);
	m_hIconEnd = LoadImageIcon(IDI_CALL_DECLINE, 16, 16);
	m_ButtonCall.m_nAlignStyle = CMFCButton::ALIGN_CENTER;
	m_ButtonCall.m_bRightImage = FALSE;
	m_ButtonCall.SetImage(m_hIconCall, FALSE, NULL, NULL, TRUE);
	m_ButtonEnd.m_nAlignStyle = CMFCButton::ALIGN_CENTER;
	m_ButtonEnd.m_bRightImage = FALSE;
	m_ButtonEnd.SetImage(m_hIconEnd, FALSE, NULL, NULL, TRUE);
...
BEGIN_MESSAGE_MAP(MessagesDlg, CBaseDialog)
	...
	ON_BN_CLICKED(IDC_CLIENT_CARD, &MessagesDlg::OnBnClickedClientCard)
...
MessagesContact* MessagesDlg::AddTab(CString number, BOOL activate, pjsua_call_info *call_info, call_user_data *user_data, BOOL notShowWindow, BOOL ifExists, CString numberOriginal, CString name)
{
	...
	if (exists == -1 || isNewCall || !name.IsEmpty()) {
...
void MessagesDlg::OnChangeTab(pjsua_call_info *p_call_info, call_user_data *user_data)
{
	...
	if (messagesContact->callId != -1) {
		...
		if (accountSettings.singleMode
			...
			CString numberLocal;
			numberLocal = messagesContact->number;
			SIPURI sipuri;
			MSIP::ParseSIPURI(numberLocal, &sipuri);
...
BOOL MessagesDlg::CloseTab(int i, BOOL safe)
{
	...
	if (messagesContact) {
		...
		if (!count) {
			...
			tab->SetCurSel(curSel < count && curSel != -1 ? curSel : count - 1);
			OnChangeTab();
		}
	}
	return TRUE;
			...
			for (unsigned i = 0; i < calls_count; ++i) {
				...
				if (user_data_curr) {
					...
					if (user_data_curr->inConference) {
						...
						pjsip_generic_string_hdr *join;
						pj_str_t hvalue, hname;
						hname = pj_str("X-Conf-Call-ID");
						hvalue = call_info_curr.call_id;
						join = new pjsip_generic_string_hdr();
	...
	if (!hidden) {
		if (status == PJ_SUCCESS) {
			...
			if (pjsua_call_get_info(call_id, &call_info) == PJ_SUCCESS) {
				...
				if (messagesContact) {
					mainDlg->pageCalls->Add(call_info.call_id, messagesContact->numberOriginal, messagesContact->name, MSIP_CALL_OUT, user_data);
				}
			}
		}
	}
	...
	if (call_id != PJSUA_INVALID_ID) {
		...
		if (status != PJ_ERESOLVE) {
			...
			if (message.IsEmpty() || PJ_STATUS_TO_OS(status) == ERROR_INVALID_PARAMETER) {
				message = Translate(_T("Allow access to the microphone in your antivirus settings."));
			}
			else if (message == _T("Undefined external error.")) {
				message = Translate(_T("Fix microphone permission in the Windows settings (Windows Settings => Privacy => Microphone)."));
			}
			else if (status == PJMEDIA_EAUD_NODEFDEV) {
				message = Translate(_T("Speakers and microphone both are required. To make calls you must have input and output sound device in your system."));
			}
...
void MessagesDlg::OnBnClickedClientCard()
{
	MessagesContact* messagesContact = GetMessageContact();
	if (!messagesContact) {
		return;
	}
	if (messagesContact->callId != PJSUA_INVALID_ID) {
		OpenClientCardForCall(messagesContact->callId);
	}
	else {
		OpenClientCard(messagesContact->numberOriginal.IsEmpty() ?
			messagesContact->number : messagesContact->numberOriginal);
	}
}
...
void MessagesDlg::OnEndCall(pjsua_call_info *call_info, call_user_data *user_data)
{
	...
	if (call_info->role == PJSIP_ROLE_UAS && call_info->connect_duration.sec == 0 && call_info->connect_duration.msec == 0) {
		...
		if (!ignore) {
            mainDlg->pageCalls->Add(call_info->call_id, number, name, elsewhere ? MSIP_CALL_ELSE : MSIP_CALL_MISS, user_data);
			if (declined) {
				info = _T("Declined");
			}
			else if (elsewhere) {
...
void MessagesDlg::UpdateCallButton(BOOL active, pjsua_call_info *call_info, call_user_data *user_data)
{
	...
	GetDlgItem(IDC_CLIENT_CARD)->ShowWindow(showClientCard ? SW_SHOW : SW_HIDE);
#ifdef _GLOBAL_VIDEO
	if (accountSettings.disableVideo) {
		GetDlgItem(IDC_VIDEO_CALL)->ShowWindow(SW_HIDE);
	}
...
BOOL MessagesDlg::SendInstantMessage(MessagesContact* messagesContact, CString message, CString number)
{
	...
	if (message.GetLength()) {
		...
		if (SelectSIPAccount(messagesContact ? messagesContact->number + messagesContact->numberParameters : number, acc_id, &pj_uri)) {
			...
			status = pjsua_im_send(acc_id, &pj_uri, NULL, &pj_str(buf), NULL, NULL);
			free(pj_uri.ptr);
			free(buf);
		}
		else {
				...
				if (action == MSIP_ACTION_ATTENDED_TRANSFER) {
					...
					if (attendedTransferFC) {
						...
						if (!accountSettings.singleMode) {
							...
							AfxMessageBox(_T("Attended transfer is not supported in single call mode. Disable single call mode or use feature codes."));
						}
					}
					return true;
				}
...
bool MessagesDlg::GotoTab(int i, CTabCtrl* tab) {
	if (!tab) {
		tab = (CTabCtrl*)GetDlgItem(IDC_MESSAGES_TAB);
	}
